...
**Closeout timestamp (UTC):** 2026-01-31T15:52:00Z  
**SPEC_VERSION:** 1.0.0  
**CANON_VERSION:** 2  
...
| Version | Date | Event |
|---------|------|-------|
| 1 | 2026-01-31 | Initial closeout (INVALID — nested duplicate corruption) |
| 2 | 2026-01-31 | Truth restoration; duplicate removed; manifest-reality parity restored |

**D1 Resolution:** Option A — Bump to v2. v1 audit chain invalidated by nested duplicate directory corruption. Hardfixes and governance kernel repair deployability, enforcement, and truth integrity.
...
- Envelope record schemas, canonicalization rules, hash semantics, failure taxonomy, conformance criteria,
  and replay semantics are **LOCKED** at SPEC_VERSION 1.0.0 / CANON_VERSION 2.
...
[ $(wc -l < REPO_MANIFEST.txt) -eq 178 ] && echo "PASS: file count" || echo "FAIL: file count"

# No nested duplicate
[ $(find . -type d -name 'CONTROL-PANEL-main' | wc -l) -eq 0 ] && echo "PASS: no duplicate" || echo "FAIL: duplicate exists"

# Authority hash verification (sample)
[ "$(sha256sum .github/workflows/ci.yml | cut -d' ' -f1)" = "c28cb8e57fde03deaeb04d8907b7e65a3e92aa446e6cb9fdd024e18902ce0525" ] && echo "PASS: ci.yml" || echo "FAIL: ci.yml"
```

---

*This closeout establishes CANON_VERSION 2 following D1 resolution. v1 audit chain is invalidated.*
