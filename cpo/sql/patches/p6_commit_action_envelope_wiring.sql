-- p6_commit_action_envelope_wiring.sql
-- Phase 6, Task 6.2: Wire envelope persistence into commit_action (write aperture)
--
-- Purpose:
--   - Enforce Write Aperture invariant: envelopes cannot be written via side-channel.
--   - Enforce Schema–Persistence alignment: if contract allows 'envelopes', commit_action persists them.
--   - Enforce Hash Coherence: DB recomputes SHA-256 over canonical JSON and fail-closes on mismatch.
--
-- NOTE:
--   - This patch assumes p6_envelope_persistence.sql has already created:
--       * cpo.cpo_envelopes
--       * cpo.canonicalize_jsonb(jsonb)
--       * pgcrypto / public.digest()
--
BEGIN;
...
-- P6: envelopes persistence (hash coherence)
v_env_elem jsonb;
v_env jsonb;
v_env_declared_hash text;
v_env_record_type text;
v_env_canonical text;
v_env_computed_sha256 text;
v_env_id uuid;
...
-- Phase 6: envelope persistence (hash coherence, fail-closed)
IF (p_artifacts ? 'envelopes') THEN
  FOR v_env_elem IN SELECT value FROM jsonb_array_elements(p_artifacts->'envelopes')
  LOOP
    v_env_declared_hash := v_env_elem->>'envelope_hash';
    v_env_record_type := v_env_elem->>'record_type';
    v_env := v_env_elem->'envelope';

    IF v_env IS NULL THEN
      RAISE EXCEPTION 'ENVELOPE_MISSING_PAYLOAD: artifacts.envelopes[].envelope required'
        USING ERRCODE = '22023';
    END IF;

    -- record_type must exist and match
    IF v_env_record_type IS NULL OR v_env_record_type = '' THEN
      RAISE EXCEPTION 'ENVELOPE_MISSING_RECORD_TYPE: artifacts.envelopes[].record_type required'
        USING ERRCODE = '22023';
    END IF;

    IF (v_env->>'record_type') IS NULL OR (v_env->>'record_type') = '' THEN
      RAISE EXCEPTION 'ENVELOPE_MISSING_RECORD_TYPE: envelope must contain record_type'
        USING ERRCODE = 'CPO70';
    END IF;

    IF (v_env->>'record_type') <> v_env_record_type THEN
      RAISE EXCEPTION 'ENVELOPE_RECORD_TYPE_MISMATCH: artifact=% envelope=%', v_env_record_type, (v_env->>'record_type')
        USING ERRCODE = '22023';
    END IF;

    -- Canonicalize and hash (must match Keystone JS)
    v_env_canonical := cpo.canonicalize_jsonb(v_env);
    v_env_computed_sha256 := encode(public.digest(convert_to(v_env_canonical, 'UTF8'), 'sha256'), 'hex');

    -- Hash coherence check (fail-closed)
    IF v_env_declared_hash IS NOT NULL AND v_env_declared_hash <> v_env_computed_sha256 THEN
      RAISE EXCEPTION 'ENVELOPE_HASH_MISMATCH: declared=% computed=%', v_env_declared_hash, v_env_computed_sha256
        USING ERRCODE = 'CPO71',
              DETAIL = 'Keystone-declared hash does not match DB-computed hash. Canonicalization divergence detected.',
              HINT = 'declared=' || v_env_declared_hash || ' computed=' || v_env_computed_sha256;
    END IF;

    -- Insert into envelope table under commit_action write aperture
    v_env_id := public.gen_random_uuid();
    INSERT INTO cpo.cpo_envelopes (
      id, agent_id, action_log_id, envelope_hash, record_type,
      canonical_json, computed_sha256, envelope
    ) VALUES (
      v_env_id, p_agent_id, v_action_log_id, v_env_computed_sha256, v_env_record_type,
      v_env_canonical, v_env_computed_sha256, v_env
    );
  END LOOP;
END IF;
