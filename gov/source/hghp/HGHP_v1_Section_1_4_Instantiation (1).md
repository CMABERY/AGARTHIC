...
**Status:** INSTANTIATED — enum populated, scope ledger initialized, C-2/C-3 ready to execute  
...
| **Payload fields to inspect** | `intent_type`, `invoice_path`, `vendor_record_path`, `validation_parameters` (if present), `notes` |
...
| **Payload fields to inspect** | `intent_type`, `invoice_path`, `vendor_record_path`, `reconciliation_parameters` (if present), `claimed_effect`, `notes` |
...
| **Payload fields to inspect** | `intent_type`, `evidence_hash`, `evidence_path` (or CAS reference), `source_context` (execution token reference, run ID, or provenance chain), `content_type`, `notes` |
...
| Condition | Description | Status | Blocking? |
|---|---|---|---|
| **C-1** | Rubric instantiated for every intent type | **COMPLETE** — 3/3 intents populated, 100% coverage | No |
| **C-2** | Historical validation (≥10 proposals) | **READY TO EXECUTE** — packet in §4 | Yes |
| **C-3** | Two-reviewer consistency (5 proposals, ≥80%) | **READY TO EXECUTE** — packet in §5 | Yes |
| **C-4** | Anti-overload controls implemented | **OPEN** — rate limits, queue hygiene, staleness rules need implementation evidence | Yes |
| **C-5** | Adversarial examples reviewed by team | **OPEN** — AE-01 through AE-07 training pending | Yes |
| **C-6** | Scope ledger created | **COMPLETE** — v0 initialized in §3, hash sealed | No |

**Path to HGHP-v1 = CLOSED:** Execute C-2 → Execute C-3 → Implement C-4 → Deliver C-5 training → All six met → CLOSED.

---

*HGHP-v1 §1.4 Instantiation. Document ID: HGHP-v1-S1.4. Status: INSTANTIATED. Intent enum: 3 values, triple-coupled, drift-enforced. Enum hash: `b21c33c1bde662570ce204af12a8ce8f7ed33c5861539e8bc871770a2b3b00b8`. Scope ledger: v0. C-1 and C-6: COMPLETE. C-2 and C-3: READY TO EXECUTE.*
