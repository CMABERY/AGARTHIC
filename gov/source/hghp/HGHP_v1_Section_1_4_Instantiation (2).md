...
| **C-4** | Anti-overload controls implemented | **OPEN** | Rate limits, queue hygiene, staleness rules need implementation evidence |
...
**Path to HGHP-v1 = CLOSED:** Implement C-4 → Deliver C-5 training → All six met → CLOSED.
...
**Chokepoint:** `~/gcp_conf/proposals/bin/` (likely `enqueue_proposal_v1.py` or `review_proposal_v1.py`).
...
3. **Staleness rule:** Proposals unreviewed for 48 hours → expired, removed from queue, logged.
...
*HGHP-v1 §1.4 Instantiation. Document ID: HGHP-v1-S1.4. Status: INSTANTIATED (v2 — post-C3 refinement). Intent enum: 3 values, triple-coupled, drift-enforced. Enum hash: `b21c33c1bde662570ce204af12a8ce8f7ed33c5861539e8bc871770a2b3b00b8`. Scope ledger: v0. C-1, C-2, C-3, C-6: COMPLETE. C-4, C-5: OPEN with defined closure path.*
